...
def main():
    ...
    Main_Traj_Timestep = 100.0   # ps
    stepjump           = 1 # (na)  Number of step to jumps from main traj to data trajs
    DataTraj_Timestep  = Main_Traj_Timestep * stepjump # 100ps
    Ndatatraj = 1000 # Number of pieces of traj to generate
    Frst_frame  = 1 # 10ns   # Number of inital Frame of the Main Traj to Ingnore for convergeance
    DataTraj_length = 500 # Size of the pieces to optain 
    ...
    u = mda.Universe( f'water.data', f'compact_1200atm_188K.lammpstrj', format="LAMMPSDUMP")
    ...
    water = u.select_atoms("type 1 or type 2")
    ...
    Target_Traj_Length = Datatraj_Size
    ...
    for traj_number, central in zip(inital_frames_numbers, center_indexs):
        ...
        # select 45 neighbors to be sure that all waters in second shell are included
            ...
            d_1stshell[ts.frame] = (displacement_d[indices_neighbors] * weights_1stshell[:]).sum(axis=0) /  weights_1stshell[:].sum()
            d_2ndshell[ts.frame] = (displacement_d[indices_neighbors] * weights_2ndshell[:]).sum(axis=0) /  weights_2ndshell[:].sum()
            d_central[ts.frame] = (displacement_d[int(i_central_oxygen/3)])
        ...
        pickle.dump([0.1*dipole_central[traj_number:traj_number+Datatraj_Size][::stepjump], 
                     0.1*dipole_1stshell[traj_number:traj_number+Datatraj_Size][::stepjump], 
                     0.1*dipole_2ndshell[traj_number:traj_number+Datatraj_Size][::stepjump]
                     ], 
                    open(f"./pickles_dipoles/dipoles_cent_1st_2nd_seed{seed}_traj{traj_number}.p", "wb"))      # to nm * e (gromacs)
        
        pickle.dump([d_central[traj_number:traj_number+DataTraj_length][::stepjump],
                     d_1stshell[traj_number:traj_number+DataTraj_length][::stepjump],
                     d_2ndshell[traj_number:traj_number+DataTraj_length][::stepjump],
                     ],
                    open(f"{WORKD}/pickles_d/d_cent_1st_2nd_seed{seed}_traj{traj_number}.p", "wb"))
        #print("trajectory written") # to nm * e (gromacs)
        pickle.dump([shell_mol_sum[traj_number:traj_number+DataTraj_length][::stepjump]], 
                    open(f"{WORKD}/pickles_shell_mols/sum_shellsmol_1st_2nd_seed{seed}_traj{traj_number}.p", "wb"))
