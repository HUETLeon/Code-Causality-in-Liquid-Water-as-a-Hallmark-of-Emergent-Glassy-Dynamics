...
def main():
    ...
    eval_set = [i for i in range(0,500+1,100)] + [i for i in range(0,100+1,10)] + [i for i in range(0,10+1,1)] 
    for i in eval_set:
        discard_close_ind =i
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(1,3,figsize=(12,3))
        indices = np.arange(len(nn_indices_center))
        ax1.hist2d(indices, nn_indices_center, bins=50)
        ax2.hist2d(indices, nn_indices_1st, bins=50)
        ax3.hist2d(indices, nn_indices_2nd, bins=50)
        ax1.set(xlabel="Point index", ylabel="nn index", title = 'ref')
        ax2.set(xlabel="Point index", title = '1st')
        ax3.set(xlabel="Point index", title = '2nd')
        fig.suptitle(f"LD: dci={i}")
        plt.tight_layout()
        plt.savefig(f"RANKS/Rank_map_E{E}_Disc{discard_close_ind}_hist.jpg")
