...
def main():
    ...
    dipole_central_cor = np.zeros((Correlation_size, Nseed))
    dipole_1stshell_cor = np.zeros((Correlation_size, Nseed))
    dipole_2ndshell_cor = np.zeros((Correlation_size, Nseed))
    ...
    print(f"shape dipole_central : {dipole_central.shape}")
    print(f"shape dipole_central_cor : {dipole_central_cor.shape}")

    for seed in tqdm(range(Nseed)):
        time, dipole_central_cor[:,seed] = autocorrelation_1d( dipole_central[:,seed][::stepjump], timestep = timestep * stepjump, cutoff= Correlation_size)
        time, dipole_1stshell_cor[:,seed] = autocorrelation_1d( dipole_1stshell[:,seed][::stepjump], timestep = timestep * stepjump, cutoff= Correlation_size)
        time, dipole_2ndshell_cor[:,seed] = autocorrelation_1d( dipole_2ndshell[:,seed][::stepjump], timestep = timestep * stepjump, cutoff= Correlation_size)
        time, dipole_out_1stshell_cor[:,seed] = autocorrelation_1d( dipole_out_1stshell[:,seed][::stepjump], timestep = timestep * stepjump, cutoff= Correlation_size)
        time, dipole_out_2ndshell_cor[:,seed] = autocorrelation_1d( dipole_out_2ndshell[:,seed][::stepjump], timestep = timestep * stepjump, cutoff= Correlation_size)
    ...
    np.savetxt( "dipole_1stshell_cor_mean_dumb.txt", dipole_1stshell_cor_mean)
    np.savetxt( "dipole_2ndshell_cor_mean_dumb.txt", dipole_2ndshell_cor_mean)
    ...
    dipole_central_cor_std = dipole_central_cor.std(axis=1)/np.sqrt(Nseed)
    dipole_1stshell_cor_std = dipole_1stshell_cor.std(axis=1)/np.sqrt(Nseed)
    dipole_2ndshell_cor_std = dipole_2ndshell_cor.std(axis=1)/np.sqrt(Nseed)
    ...
    time_and_dipole_central_cor_mean = np.array([time, dipole_central_cor_mean, dipole_central_cor_std]).T
    time_and_dipole_1stshell_cor_mean = np.array([time, dipole_1stshell_cor_mean, dipole_1stshell_cor_std]).T
    time_and_dipole_2ndshell_cor_mean = np.array([time, dipole_2ndshell_cor_mean, dipole_2ndshell_cor_std]).T
    ...
    np.savetxt( "dipole_1stshell_cor_mean_v3.txt", time_and_dipole_1stshell_cor_mean)
    np.savetxt( "dipole_2ndshell_cor_mean_v3.txt", time_and_dipole_2ndshell_cor_mean)
