...
def compute_info_imbalance_causality(dists2_cause_present, 
                                     dists2_effect_present,
                                     ranks_effect_future,
                                     alpha, 
                                     k):
    """
    ...
    ranks_present = _compute_rank_matrix(
        alpha * alpha * dists2_cause_present + dists2_effect_present
    )
...
def _return_mask(npoints, discard_close_ind=0):
    ...
    """
    mask = np.abs(
        np.arange(npoints)[:, np.newaxis] - np.arange(npoints)[np.newaxis, :]
    )
...
def scan_alphas(cause_present, 
    ...
    ranks_effect_future = _compute_rank_matrix(
        _compute_dist2_matrix(effect_future, mask=mask)
    )

    info_imbalances = Parallel(n_jobs=n_jobs)(delayed(
        compute_info_imbalance_causality
            )(
