import numpy as np
import matplotlib.pyplot as plt
from scipy.spatial import KDTree
import MDAnalysis as mda
from scipy.spatial import cKDTree
import pickle
from tqdm import tqdm

import argparse
import sys
colors = ["#FF595E","#8AC926","#1982C4","black"]


from autocorrelation import autocorrelation_1d

def cut_after(dists, d0, d_width=0.1):
    return 1. / (1. + np.exp((dists-d0) / d_width))

def cut_before(dists, d0, d_width=0.1):
    return 1. / (1. + np.exp(-(dists-d0) / d_width))

def compute_weights_1stshell(dists, cutoff_1st, d_width):
    return cut_after(dists, d0=cutoff_1st, d_width=d_width)

def compute_weights_2ndshell(dists, cutoff_1st, cutoff_2nd, d_width):
    return (-1. + cut_before(dists, d0=cutoff_1st, d_width=d_width)
               + cut_after(dists, d0=cutoff_2nd, d_width=d_width))

def cut_cos_front(coss, cos0 = 0, cos_width=0.1):
    return 1. / (1. + np.exp(-(coss - cos0) / cos_width))

def compute_weights_1stshell_front(dists, coss, cutoff_1st, d_width, cos_width):
    return (cut_after(dists, d0=cutoff_1st, d_width=d_width)
            * cut_cos_front(coss, cos_width=cos_width)
            )

def compute_weights_1stshell_back(dists, coss, cutoff_1st, d_width, cos_width):
    return ( cut_after(dists, d0=cutoff_1st, d_width=d_width)
            * (1. - cut_cos_front(coss, cos_width=cos_width))
            )

def compute_weights_2ndshell_front(dists, coss, cutoff_1st, cutoff_2nd, d_width, cos_width):
    return  (compute_weights_2ndshell(dists, cutoff_1st, cutoff_2nd, d_width)
            * cut_cos_front(coss, cos_width=cos_width)
            )

def compute_weights_2ndshell_back(dists, coss, cutoff_1st, cutoff_2nd, d_width, cos_width):
    return  (compute_weights_2ndshell(dists, cutoff_1st, cutoff_2nd, d_width)
            * (1. - cut_cos_front(coss, cos_width=cos_width))
            )

def main():

    # read random seed and number of trajectories (from 1 to 1962)

    # read trajectory with MDAnalysis
    u = mda.Universe(f'conf.gro',
                     f'300w_300K_1atm.xtc')
    water = u.select_atoms("type O or type H")
    natoms = water.n_atoms
    Nframes = len(u.trajectory)
    Nseed = 50

    print(f"Number of atoms: {natoms}\nNumber of frames: {Nframes}")
    assert natoms % 3 == 0, "System does not contain only water molecules"

    # set arrays
    indices_oxygens = np.arange(0,natoms,3, dtype=int)
    indices_hydrogens = np.column_stack((np.arange(1,natoms,3, dtype=int),
                                         np.arange(2,natoms,3, dtype=int)))
    assert indices_oxygens.shape[0] == indices_hydrogens.shape[0], "Number of O and H atoms does not match"
    
    # set cutoff distances defining first and second shell (in angstrom)
    cutoff_1st = 3.3
    cutoff_2nd = 5.7
    d_width = 0.1 # this is only to set the width of the switching function
    #print(indices_oxygens)

    corelation_time = 2000 # ps
    ingnoreddata = 85 # %
    timesetp = 10 # fs
    jumpstep = 50
    Correlation_size =  int(corelation_time * 1000 / (timesetp*jumpstep))
    output_time = timesetp * jumpstep

    indices_reference_oxygens = np.random.permutation(indices_oxygens)[:Nseed]
    dipole_central = np.zeros((Nframes, Nseed ,3))
    density_1stshell = np.zeros((Nframes, Nseed))
    density_2ndshell = np.zeros((Nframes, Nseed))
    for ts in tqdm(u.trajectory[Nframes*ingnoreddata//100:][::jumpstep]):
        for i, i_central_oxygen in enumerate(indices_reference_oxygens):

            coordinates = water.positions # updated automatically in the loop
            box = water.dimensions[:3] # 3 unit cell dimensions (orthogonal box)
            delta = 0.415
            ######################### COMPUTE ALL DIPOLES #########################

            coord_diff = coordinates[i_central_oxygen,np.newaxis] - coordinates[indices_oxygens]
            coord_diff = coord_diff - np.around(coord_diff / box) * box
            dist_iO_from_allO = np.sqrt(np.sum(coord_diff*coord_diff, axis=1))

            indices_neighbors = np.argpartition(dist_iO_from_allO, np.arange(45))[1:45]
            dist_iO_from_allO = dist_iO_from_allO[indices_neighbors]

            vectors_HO = coordinates[indices_oxygens,np.newaxis] - coordinates[indices_hydrogens]
            vectors_HO[:,0] = vectors_HO[:,0] - np.around(vectors_HO[:,0] / box[np.newaxis,:]) * box[np.newaxis,:]
            vectors_HO[:,1] = vectors_HO[:,1] - np.around(vectors_HO[:,1] / box[np.newaxis,:]) * box[np.newaxis,:]
            dipoles = delta * vectors_HO.sum(axis=1)
        
            weights_1stshell = compute_weights_1stshell(
                dists = dist_iO_from_allO,
                cutoff_1st = cutoff_1st,
                d_width = d_width,
            )

            weights_2ndshell = compute_weights_2ndshell(
                dists=dist_iO_from_allO,
                cutoff_1st=cutoff_1st,
                cutoff_2nd=cutoff_2nd,
                d_width=d_width,
            )

            density_1stshell[ts.frame, i] = weights_1stshell.sum()
            density_2ndshell[ts.frame, i] = weights_2ndshell.sum()


    density_1stshell_cor = np.zeros((Correlation_size, Nseed))
    density_2ndshell_cor = np.zeros((Correlation_size, Nseed))
    print(f"length before reshape : {len(density_1stshell[:,0])}")

    density_1stshell = density_1stshell[Nframes*ingnoreddata//100:,:][::jumpstep,:]
    density_2ndshell = density_2ndshell[Nframes*ingnoreddata//100:,:][::jumpstep,:]

    print(f"length after reshape : {len(density_1stshell[:,0])}")

    for seed in tqdm(range(Nseed)):
        time, density_1stshell_cor[:,seed] = autocorrelation_1d(density_1stshell[:,seed], timestep = timesetp * jumpstep , cutoff= Correlation_size)
        time, density_2ndshell_cor[:,seed] = autocorrelation_1d(density_2ndshell[:,seed], timestep = timesetp * jumpstep , cutoff= Correlation_size)
    
    density_1stshell_cor_mean = density_1stshell_cor.mean(axis = 1)
    density_2ndshell_cor_mean = density_2ndshell_cor.mean(axis = 1)

    density_1stshell_cor_std = density_1stshell_cor.std(axis = 1)
    density_2ndshell_cor_std = density_2ndshell_cor.std(axis = 1)


    time_and_density_1stshell_cor = np.hstack([time[:,np.newaxis], density_1stshell_cor_mean[:,np.newaxis], density_1stshell_cor_std[:,np.newaxis]])
    time_and_density_2ndshell_cor = np.hstack([time[:,np.newaxis], density_2ndshell_cor_mean[:,np.newaxis], density_2ndshell_cor_std[:,np.newaxis]])
    np.savetxt( "time_and_density_1stshell_cor.txt", time_and_density_1stshell_cor)
    np.savetxt( "time_and_density_2ndshell_cor.txt", time_and_density_2ndshell_cor)

    time_and_density_1stshell_cor_rescale = np.hstack([time[:,np.newaxis], density_1stshell_cor_mean[:,np.newaxis], density_1stshell_cor_std[:,np.newaxis]/density_1stshell_cor_std[0]])
    time_and_density_2ndshell_cor_rescale = np.hstack([time[:,np.newaxis], density_2ndshell_cor_mean[:,np.newaxis], density_2ndshell_cor_std[:,np.newaxis]/density_2ndshell_cor_std[0]])

    np.savetxt( "time_and_density_1stshell_cor_rescale.txt", time_and_density_1stshell_cor_rescale)
    np.savetxt( "time_and_density_2ndshell_cor_rescale.txt", time_and_density_2ndshell_cor_rescale)

#    fig, ax = plt.subplots()
    
    

    return

if __name__ == "__main__":
    main()
   
