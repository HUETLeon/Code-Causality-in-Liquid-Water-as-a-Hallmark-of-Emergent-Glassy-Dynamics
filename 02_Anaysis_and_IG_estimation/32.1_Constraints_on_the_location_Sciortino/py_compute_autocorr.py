...
def main():
    ...
    corelation_time = 200000 # ps
    ingnoreddata = 90 # %
    timesetp = 10 # fs
    Correlation_size =  int(corelation_time * 1000 / timesetp)
    ...
    for seed in tqdm(range(Nseed)):
        time, dipole_central_cor[:,seed,0] = autocorrelation_1d(dipole_central[:,seed,0], timestep = timesetp , cutoff= Correlation_size)
        time, dipole_central_cor[:,seed,1] = autocorrelation_1d(dipole_central[:,seed,1], timestep = timesetp , cutoff= Correlation_size)
        time, dipole_central_cor[:,seed,2] = autocorrelation_1d(dipole_central[:,seed,2], timestep = timesetp , cutoff= Correlation_size)
