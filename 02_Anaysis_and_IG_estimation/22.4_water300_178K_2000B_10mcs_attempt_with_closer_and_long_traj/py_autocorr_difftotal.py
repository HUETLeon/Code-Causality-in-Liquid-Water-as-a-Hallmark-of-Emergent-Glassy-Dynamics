...
def main():
    ...
    for ts in tqdm(u.trajectory[Nframes*ingnoreddata//100:]):
        ...
        for i, i_ref_oxygen in enumerate(indices_reference_oxygens):
            coord_diff = coordinates[i_ref_oxygen,np.newaxis] - coordinates[indices_oxygens] # only oxygens considered
            ...
            dipole_ref[ts.frame] = dipoles[int(i_ref_oxygen/3)]
            ...
            dipole_out_ref[ts.frame,i] = dipoles.sum(axis=0) - dipoles[i_ref_oxygen//3,:]
    ...
    pickle.dump(dipole_out_ref, open(f"pickles_dipoles_out_ref_oneseed.p", "wb"))
