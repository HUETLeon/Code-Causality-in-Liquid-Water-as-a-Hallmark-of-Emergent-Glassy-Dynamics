...
def construct_X_Y(Ntrajs, seed, njobs):

    (d_central, d_1stshell, d_2ndshell ) = (
            np.swapaxes(Parallel(n_jobs=njobs)(delayed(read_files_dipole)
            (namefile=f'./pickles_d/d_cent_1st_2nd_seed{seed}_traj{(traj_number*10000)+5000}.p') for traj_number in range(Ntrajs)), axis1=0, axis2=1)
    )
...
def main():
    ...
    for seed in tqdm(range(1,51)):
        (d_central, d_1stshell, d_2ndshell 
                ...
                construct_X_Y(Ntrajs=Ntrajs, seed=seed, njobs=njobs))
