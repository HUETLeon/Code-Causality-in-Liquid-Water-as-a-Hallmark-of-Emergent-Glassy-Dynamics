...
def main():
    ...
    Frst_frame  = 0 # 200ns   # Number of inital Frame of the Main Traj to Ingnore for convergeance
    DataTraj_length = 5000 # Size of the pieces to optain 
    DataTraj_timelength = DataTraj_length * DataTraj_Timestep # 50 ps
    ...
    u = mda.Universe(f'conf_end.gro', f'300w_300K_1atm.xtc')
    ...
    for traj_number, central in zip(inital_frames_numbers, center_indexs):
        ...
        dipole_out_1stshell = np.zeros((Nframes, 3))
        dipole_out_2ndshell = np.zeros((Nframes, 3))
        dipole_central = np.zeros((Nframes, 3))
        dipole_total = np.zeros((Nframes, 3))
        dipole_total_corrected = np.zeros((Nframes, 3))
        print("boostrap length= ", len(u.trajectory[traj_number:traj_number+Datatraj_Size])*10*0.001, "ps")
        ...
        # select 45 neighbors to be sure that all waters in second shell are included
            ...
            dipole_out_1stshell[ts.frame] = (dipoles[:] * weights_out_1stshell[:,np.newaxis]).sum(axis=0)
            dipole_out_2ndshell[ts.frame] = (dipoles[:] * weights_out_2ndshell[:,np.newaxis]).sum(axis=0)
            dipole_central[ts.frame] = dipoles[int(i_central_oxygen/3)]
            dipole_total[ts.frame] = dipoles.sum(axis=0)
            dipole_total_corrected[ts.frame] = dipole_total[ts.frame] - dipoles[int(i_central_oxygen/3)]
        #print(0.1*dipole_central[ts.frame])
        #print(dipole_1stshell.shape,dipole_2ndshell.shape)
        pickle.dump([0.1*dipole_central[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_1stshell[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_2ndshell[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_total[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_total_corrected[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_out_1stshell[traj_number:traj_number+Datatraj_Size][::stepjump], 0.1*dipole_out_2ndshell[traj_number:traj_number+Datatraj_Size][::stepjump]], 
                    open(f"../pickles_dipoles/dipoles_cent_1st_2nd_total_totalc_out1st_out2nd_seed{seed}_traj{traj_number}.p", "wb"))      # to nm * e (gromacs)
